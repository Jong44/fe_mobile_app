...
const BusinessScreen = ({
  ...
  return (
    <View style={globalStyles.contaner_screen}>
        ...
        <View style={{ flexDirection: 'row', gap: 10, height: 30 }}>
                    {Category.map((category, index) => (
                        <Text key={index} style={{ paddingHorizontal: 8, paddingVertical: 5, 
                            backgroundColor:  categoryActive === index ? '#03256C' : '#E5E5E5'
                            
                            , color: categoryActive === index ? 'white' : 'black'
                            
                            , borderRadius: 5 }}
                            onPress={() => filterCategory(index)}
